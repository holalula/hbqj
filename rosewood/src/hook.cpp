...
namespace hbqj {
    std::expected<bool, Error> Hook::SafeInject(const std::filesystem::path &dll_path, std::string_view process_name) {
        if (!std::filesystem::exists(dll_path)) {
            return std::unexpected(FileNotFoundError{.file_path = dll_path.string()});
        }

        return Inject(dll_path.wstring(), process_name);
    }
    ...
    std::expected<bool, Error> Hook::Inject(std::wstring_view dll_path, std::string_view process_name) {
        ...
        const auto &thread = CreateRemoteThreadEx(process, nullptr, 0,
                                                  reinterpret_cast<PTHREAD_START_ROUTINE>(load_lib_addr),
                                                  path_start_addr, 0, nullptr, nullptr);
        if (!thread) {
            VirtualFreeEx(process, path_start_addr, 0, MEM_RELEASE);
            return std::unexpected(WinAPIError{.error = GetLastError()});
        }
    ...
    std::expected<bool, Error> Hook::Unload(std::wstring_view dll_name, std::string_view process_name) {
        ...
        const auto &thread = CreateRemoteThread(process, nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(free_lib),
                                                module, 0, NULL);
        if (!thread) {
            return std::unexpected(WinAPIError{.error = GetLastError()});
        }
